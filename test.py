...
if player == 'human':
    print("*******************")
    print("Welcome to 好好娃娃机")
    score = 0
    while env.time_step <= 30:
        s = env.get_state()
        print("We have {:d} claw machines here. Current time: {:d}, current score: {:.1f}".format(env.machine_num, env.time_step, score))
        print("machine\t\t0\t1\t2")
        ...
        if r == -9999:
            ...
            if watch_result == -1:
                ...
                print("一个娃娃在这个机器里被抓到了！")
            elif watch_result > 0:
                print("目前观察到这台机器已经连续失败{:d}次了".format(watch_result))

    print("final score: {:.1f}".format(score))